...
function WorkPermitHolder() {
  ...
  const exportToExcel = () => {
    ...
    XLSX.writeFile(wb, "Work_Permit_Holder.xlsx"); // Write the file
  };
  useEffect(() => {
    (async () => {
      try {
        ...
        setUsers(response.data.filter((user) => user.Status === "Work Permit"));
      } catch (error) {
    ...
    <div>
      ...
      <div className="p-4 sm:ml-64">
        <div className="flex items-center justify-center h-48 mb-4 rounded  bg-green-500">
          <p className="text-5xl text-white">Work Permit Holder</p>
        </div>
...
export default WorkPermitHolder;
