...
function StudyPermitHolder() {
  ...
  const exportToExcel = () => {
    ...
    XLSX.writeFile(wb, "Study_Permit_Holder.xlsx"); // Write the file
  };
  useEffect(() => {
    (async () => {
      try {
        ...
        setUsers(
          response.data.filter((user) => user.Status === "Student Visa")
        );
      } catch (error) {
...
export default StudyPermitHolder;
